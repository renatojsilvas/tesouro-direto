...
**Deploy** (`.github/workflows/deploy.yml`): `test` → `e2e` → `deploy` (SSH VPS). `.env` sempre reescrito com `printf`, `git pull`, copia nginx conf, `nginx -t && reload`, `docker compose build --no-cache && up -d`, aguarda `/health`. Migrations rodam no startup do container. Nginx do host com TLS (tarefa 44, 2026-08-03): domínio `dadosdotesourodireto.com.br`, cert Let's Encrypt (Certbot `--standalone`→renovação `--webroot` + `certbot.timer`); `listen 80`→301 https (+ acme-challenge), `listen 443 ssl http2` com `Strict-Transport-Security: max-age=300`, `listen 3080` mantido como break-glass. `/`→Web, `/api/`→API; `/grafana/`, `/prometheus/`, `/api/swagger` e `/api/metrics` restritos a 127.0.0.1 (`allow 127.0.0.1; deny all` — túnel SSH; `/api/metrics` fechado na tarefa 43, 2026-07-28).
...
- **Capacidade/limites desconhecidos (sem teste de carga)**: nenhuma medição de throughput/p95 sob carga da API nem de circuitos SignalR simultâneos do Blazor Server. **Planejado (tarefa 58, 2026-08-06):** suíte k6 **sob demanda** (fora da pipeline) — fluxos quentes da API (ETag/preco-atual/histórico paginado/simulador) + cenário de circuitos SignalR do site, métricas no Grafana existente, baseline em `docs/load/`; nunca aponta para prod por default.
...
**Segurança / operação:**
5. Chave de API única compartilhada, default `CHANGE-ME-IN-PRODUCTION`; endpoints mutantes com mesma proteção que leitura (§1). **Parcial** ✔️ (tarefas 5 + 19): boot aborta em prod com chave default/vazia **ou `dev-local-key`**, e o compose falha sem `API_KEY` (sem fallback inseguro); segue aberto o compartilhamento único e a paridade leitura/escrita. **Planejado (tarefa 56, ADR, 2026-08-06):** API pública multi-cliente — keys **por cliente** (hasheadas, self-service, validadas contra tabela), login Google no site, seed de admin, rate limit por cliente; fecha o compartilhamento da chave única.
